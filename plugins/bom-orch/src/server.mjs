 ...
 *   증거 실행의 테스트 자식은 이 스탬프를 받지 **않는다**(`src/test-runner.mjs`, 2026-08-28) —
 *   대상의 테스트가 이 서버를 띄우는 것은 중첩이 아니다. 그 서버의 증거 실행 자식부터는 받는다(깊이 둘).
 *
 ...
 *   10초 + 워크트리 정리 최대 60초, 근거는 `src/deadline.mjs` 의 `MAX_WAIT_MS` 머리말) — 4.7배 차이다. 두 수가 다른 이유는 재는 질문이 다르기 때문이다:
